...
class Goal(Base):
    """
    A user-defined savings/investment target (e.g. "House down payment,
    ₹2,000,000 in 7 years"). The required monthly SIP and the growth
    trajectory towards it are always computed fresh from this row's
    target_amount/target_years plus the user's current risk-derived
    blended return -- nothing here is stored pre-calculated so it stays
    consistent if the user's spending (and therefore risk profile) shifts.
    """
