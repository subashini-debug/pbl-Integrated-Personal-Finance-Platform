...
def required_monthly_sip(target_amount: float, years: int, annual_return: float) -> float:
    """
    Inverse of project_growth: given a target future value and horizon,
    solve for the level monthly contribution that reaches it.

    project_growth compounds as b_m = b_(m-1) * (1 + i) + c, b_0 = 0, which
    closed-form is b_n = c * ((1+i)^n - 1) / i (future value of an ordinary
    annuity). Solving for c gives the formula below; the i == 0 edge case
    (e.g. an all-cash allocation with a ~0 rate) falls back to a straight
    linear split since the annuity formula divides by zero there.
    """
    ...
    return round(target_amount / factor, 2)


def goal_progress(target_amount: float, years: int, annual_return: float, current_monthly_surplus: float) -> dict:
    """
    Bundles what the UI needs to render one goal: the SIP required to hit
    the target on schedule, whether the user's current surplus already
    covers it, and a year-by-year projection line (capped at the goal's
    horizon) so it can be drawn on the same chart as the general roadmap.
    """
    required = required_monthly_sip(target_amount, years, annual_return)
    contribution_used = max(current_monthly_surplus, 0.0)
    on_track = contribution_used >= required
    projection = project_growth(contribution_used if contribution_used > 0 else required, years, annual_return)
    return {
        "required_monthly_sip": required,
        "on_track": on_track,
        "projection_at_current_surplus": projection,
    }
